#!/usr/bin/env python3
"""
2D Heat Diffusion with MPI (NumPy version)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="2D Heat Diffusion with MPI (NumPy version)",
...
def compute_distance(X, Y, cx, cy):
    """
    Compute Euclidean distance from each point (X[i,j], Y[i,j]) to center (cx, cy).

    Uses sqrt -- expensive operation that could be optimized in some cases.

    This is vectorized using NumPy, but still recomputed every timestep.
    """
    delta_x = X - cx
    delta_y = Y - cy
...
def compute_angle(X, Y, cx, cy):
    """
    Compute angle (in radians) from center (cx, cy) to each point (X[i,j], Y[i,j]).
...
def compute_source_term(X, Y, Lx, Ly, t):
    """
    Compute the heat source term Q(x, y, t) using vectorized NumPy operations.
    ...
    *** THIS FUNCTION IS DELIBERATELY UNOPTIMIZED ***
    ...
    Even though we use vectorized NumPy operations instead of explicit loops,
    we still recompute every expensive math operation from scratch on every call.
    ...
    Optimization opportunities:
        ...
        - Cache the distance and angle arrays instead of recomputing
    """
    ...
    # Assemble source -- uses sin, cos, exp (all vectorized)
    Q = (np.sin(np.pi * X / Lx)
         * np.cos(np.pi * Y / Ly)
         * np.exp(-r / SIGMA)
...
def main():
    ...
    if rank == 0:
        print("=" * 62)
        print("  2D Heat Diffusion with MPI (NumPy version)")
        print("=" * 62)
    ...
    for _ in range(5):
        ...
        t_phys = 0.0
        Q = compute_source_term(X, Y, domain.Lx, domain.Ly, t_phys)
